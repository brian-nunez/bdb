...
func TestNewSuccessFile(t *testing.T) {
	db, mock, err := sqlmock.New(sqlmock.MonitorPingsOption(true))
	if err != nil {
		t.Fatalf("sqlmock new failed: %v", err)
	}
	...
	openDB = func(driverName string, dataSourceName string) (*sql.DB, error) {
		if !strings.Contains(dataSourceName, "test.db") {
			t.Fatalf("expected test.db, got %s", dataSourceName)
		}
		return db, nil
	}
	...
	conn, err := New(Config{Path: "test.db"})
	if err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	if conn == nil {
